...
resource "aws_cognito_user_pool_client" "main_user_pool_client" {
  ...
  callback_urls = ["http://localhost:3000/auth/callback", "https://www.rent-a-car-cloud.click/auth/callback"]
  # Do the same for logout urls, can be changed later
  logout_urls = ["http://localhost:3000/auth/logout", "https://www.rent-a-car-cloud.click/auth/logout"]
